...
impl Stream {
    ...
    pub fn get_all_with_sender(pubkey: &str, conn: &PgConnection) -> Vec<Stream> {
    ...
    pub fn get_all_with_receiver(pubkey: &str, conn: &PgConnection) -> Vec<Stream> {
        use crate::schema::streams::dsl::*;
        streams
    ...
    fn id_is_present(id: &str, conn: &PgConnection) -> bool {
        use crate::schema::streams::dsl::*;
        matches!(streams.find(id).first::<Stream>(conn), Ok(_s))
    }
